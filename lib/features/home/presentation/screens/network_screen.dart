          ...
          itemBuilder: (context, index) {
            ...
            return FutureBuilder<UserModel?>(
              ...
              builder: (context, userSnap) {
                 if (!userSnap.hasData) return const SizedBox(height: 80, child: AppLoadingIndicator());
                 ...
                 return FadeInUp(
                   ...
                   child: NetworkRequestCard(
                     user: userSnap.data!,
                     onAccept: () async {
                       await firestoreService.acceptFriendRequest(req.id, currentUserId, userSnap.data!.id);
                     },
                     onDecline: () async {
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(
           ...
           itemBuilder: (context, index) {
              final req = requests[index];
              return FutureBuilder<UserModel?>(
                future: firestoreService.getUser(req['to']), // 'to' is the receiver
                builder: (context, userSnap) {
                   if (!userSnap.hasData) return const SizedBox(height: 80, child: AppLoadingIndicator());
